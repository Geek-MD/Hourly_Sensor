...
- Restore a dedicated device for every configured hourly sensor so its sensor
  and button remain visible together on the integration page.
- Link each dedicated device to the physical source device with Home Assistant's
  `via_device` relationship instead of assigning the generated entities directly
...
- Associate each generated rolling sensor and recalculation button with the
  source entity's device, automatically placing them in that device's area.
- Leave generated entities without a device when the selected source entity is
  not itself associated with one.
